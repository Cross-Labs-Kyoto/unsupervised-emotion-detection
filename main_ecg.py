...
from datasets import load_data_ecg, DreamerDataset, TripletSampler, DatasetType
...
if __name__ == "__main__":
    ...
    if args.lstm:
        ...
        model = ContrastiveLSTM(in_size=FACED['channels'] * len(BANDS), hid_lstm=5, hid_fc=[20, 20],
                                out_size=3, l_rate=args.l_rate, batch_size=args.batch_size, dropout=args.dropout)
    else:
    ...
    data , labels, _, _, nb_subs = load_data_ecg()

    if args.infer:
        # Build dataset
        logger.info("Creating dataset")
        emo_ds = DreamerDataset(data, labels)
        ...
        train_ds = DreamerDataset(data, labels)
        val_ds = DreamerDataset(data, labels)
